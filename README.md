...
#### 130. Surrounded Region (2D DFS 구현)
```cpp
// Surrounded Region
// 일반적인 2D 행렬에서의 DFS
void dfs(vector<vector<char>> &grid, int cur_row, int cur_column)
{
    ...
    // Base case1: 바깥으로 나간경우
    if (cur_row < 0 or cur_row >= grid.size() or cur_column < 0 or cur_column >= grid[0].size())
    {
        return;
    }

    // 만약에 'O'가 아닌 경우
    if (grid[cur_row][cur_column] == 'X' or grid[cur_row][cur_column] == 'T')
    {
        return;
    }
    // 'T': 바깥에서 도달가능
